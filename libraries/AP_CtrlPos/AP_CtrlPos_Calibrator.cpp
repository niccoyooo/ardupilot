...
const uint32_t AP_CTRLPOS_CAL_TIMEOUT_SEC = 120;        // calibration timesout after 120 seconds
const uint32_t AP_CTRLPOS_CAL_STATUSINTERVAL_SEC = 3;   // status updates printed at 3 second intervals
const float AP_CTRLPOS_CAL_YAW_MAX_RADS = radians(20);  // maximum yaw rotation (must be low to ensure good scaling)
const float AP_CTRLPOS_CAL_ROLLPITCH_MIN_RADS = radians(20);    // minimum acceptable roll or pitch rotation
const float AP_CTRLPOS_CAL_SCALE_MIN = 0.20f;           // min acceptable scaling value.  If resulting scaling is below this then the calibration fails
const float AP_CTRLPOS_CAL_SCALE_MAX = 4.0f;            // max acceptable scaling value.  If resulting scaling is above this then the calibration fails
const float AP_CTRLPOS_CAL_FITNESS_THRESH = 0.5f;       // min acceptable fitness
const float AP_CTRLPOS_CAL_RMS_FAILED = 1.0e30f;        // calc_mean_squared_residuals returns this value when it fails to calculate a good value

extern const AP_HAL::HAL& hal;

// start the calibration
void AP_CtrlPos_Calibrator::start()
{
...
void AP_CtrlPos_Calibrator::stop()
{
...
bool AP_CtrlPos_Calibrator::update()
{
    ...
    if (_cal_state == CalState::RUNNING) {
        ...
        if (AP::ahrs().getOptFlowSample(timestamp_ms, flow_rate, body_rate, los_pred)) {
            ...
            if (now_ms - _last_report_ms > AP_CTRLPOS_CAL_STATUSINTERVAL_SEC * 1000UL) {
                _last_report_ms = now_ms;
                GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s x:%d%% y:%d%%",
                                prefix_str,
                                (int)((_cal_data[0].num_samples * 100.0 / AP_CTRLPOS_CAL_MAX_SAMPLES)),
                                (int)((_cal_data[1].num_samples * 100.0 / AP_CTRLPOS_CAL_MAX_SAMPLES)));
            }
        ...
        // check for timeout
        if (now_ms - _start_time_ms > AP_CTRLPOS_CAL_TIMEOUT_SEC * 1000UL) {
...
Vector2f AP_CtrlPos_Calibrator::get_scalars()
{
...
void AP_CtrlPos_Calibrator::add_sample(uint32_t timestamp_ms, const Vector2f& flow_rate, const Vector2f& body_rate, const Vector2f& los_pred)
{
    ...
    if (fabsf(gyro.z) > AP_CTRLPOS_CAL_YAW_MAX_RADS) {
        return;
    }

    // check enough roll or pitch movement and record sample
    const bool rates_x_sufficient = (fabsf(body_rate.x) >= AP_CTRLPOS_CAL_ROLLPITCH_MIN_RADS) && (fabsf(flow_rate.x) >= AP_CTRLPOS_CAL_ROLLPITCH_MIN_RADS);
    ...
    const bool rates_y_sufficient = (fabsf(body_rate.y) >= AP_CTRLPOS_CAL_ROLLPITCH_MIN_RADS) && (fabsf(flow_rate.y) >= AP_CTRLPOS_CAL_ROLLPITCH_MIN_RADS);
...
bool AP_CtrlPos_Calibrator::sample_buffers_full() const
{
...
bool AP_CtrlPos_Calibrator::run_calibration()
{
...
bool AP_CtrlPos_Calibrator::calc_scalars(uint8_t axis, float& scalar, float& fitness)
{
    // prefix for reporting
    const char* prefix_str = "FlowCal:";
    ...
    if (best_scalar_total < AP_CTRLPOS_CAL_SCALE_MIN) {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s scalar%s:%4.3f too low (<%3.1f)", prefix_str, axis_str, (double)best_scalar_total, (double)AP_CTRLPOS_CAL_SCALE_MIN);
        return false;
    }
    if (best_scalar_total > AP_CTRLPOS_CAL_SCALE_MAX) {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s scalar%s:%4.3f too high (>%3.1f)", prefix_str, axis_str, (double)best_scalar_total, (double)AP_CTRLPOS_CAL_SCALE_MAX);
        return false;
    }

    // check for poor fitness
    float fitness_new = calc_mean_squared_residuals(axis, best_scalar_total);
    if (fitness_new > AP_CTRLPOS_CAL_FITNESS_THRESH) {
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "%s scalar%s:%4.3f fit:%4.3f too high (>%3.1f)", prefix_str, axis_str, (double)scalar, (double)fitness_new, (double)AP_CTRLPOS_CAL_FITNESS_THRESH);
        return false;
    }
...
float AP_CtrlPos_Calibrator::calc_sample_residual(const sample_t& sample, float scalar) const
{
...
bool AP_CtrlPos_Calibrator::calc_sample_best_scalar(const sample_t& sample, float& best_scalar) const
{
...
float AP_CtrlPos_Calibrator::calc_mean_squared_residuals(uint8_t axis, float scalar) const
{
    // sanity check axis
    if (axis >= 2) {
        return AP_CTRLPOS_CAL_RMS_FAILED;
    }
...
// log all samples
void AP_CtrlPos_Calibrator::log_sample(uint8_t axis, uint8_t sample_num, float flow_rate, float body_rate, float los_pred)
{
    // @LoggerMessage: OFCA
